...
function typeText() {
    ...
    if (isDeleting) {
        ...
        typingElement.textContent = currentText.text.substring(0, currentCharIndex - 1);
        currentCharIndex--;
...
function sendToWhatsApp(name, email, message) {
    ...
    const whatsappMessage = `Halo, saya ${name}%0A` +
                           `Email: ${email}%0A%0A` +
                           `Pesan:%0A${message}`;
...
function setupScrollAnimations() {
    ...
    const animatedElements = document.querySelectorAll('.section-title, .section-subtitle, .about-text, .contact-info, .contact-form');
